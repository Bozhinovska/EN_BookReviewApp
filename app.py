...
def register():
    if request.method == 'POST':
        ...
        if user:
            flash('This email is already in use. Try a different one.')
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        flash('The profile is successfully updated')
...
def book_details(book_id):
    ...
    if request.method == 'POST':
        ...
        if review_id:
            ...
            conn.commit()
            flash('The review has been successfully edited!')
            conn.close()
...
def add_review(book_id):
    ...
    flash('The review has been successfully added!')
    return redirect(url_for('book_details', book_id=book_id))
...
def add_favorite(book_id):
    if 'user_id' not in session:
        flash('You must be logged in to add a book to favorites.')
        return redirect(url_for('login'))
    ...
    flash('The book has been added to your favorites list')
    return redirect(url_for('book_details', book_id=book_id))
...
def remove_favorite(book_id):
    if 'user_id' not in session:
        flash('You must be logged in to remove a book from favorites.')
        return redirect(url_for('login'))
    ...
    flash('The book has been successfully removed from favorites.')
    return redirect(url_for('my_profile'))
